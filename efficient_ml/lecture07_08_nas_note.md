...
> **課程**：MIT 6.5940 TinyML and Efficient Deep Learning Computing, Fall 2023 — Lecture 7 & 8
> **講者**：Song Han
> **來源**：
> - 課堂逐字稿：[Lecture 7 錄影](https://youtu.be/W84MKJSg90A)（1:14:25）、
>   [Lecture 8 錄影](https://youtu.be/MOMc8KkGwCc)（1:14:49）
> - 官方投影片：`Lec07-Neural-Architecture-Search-I.pdf`（76 頁）、`Lec08-Neural-Architecture-Search-II.pdf`（105 頁）
> - 作業對照：`Lab3_zh.md`、`hw3.py`
>
> **標記說明**（同 [Lecture 9 筆記](lecture09_kd_note.md) 的慣例）
> - `【口頭】` = 教授課堂口頭補充、投影片沒有的內容
> - `【Q&A】` = 學生提問與回答
> - `【投影片新增】` = 投影片為後續學期更新版本，2023 課堂逐字稿沒有這段
> - 未標記者 = 投影片本身的內容，或 Lab 3 程式碼的對照
>
...
硬體成本：A100 節點約 16 萬美元、H100 節點 25–30 萬美元。

#### 【口頭】教授自己實驗室的插座故事

> 「最近我在升級實驗室的伺服器。我們原本有**兩個插頭**，每個都很粗，像你的手一樣粗。你猜這兩個插頭能撐幾台伺服器？——**10 台**。
> 那換成 A100 呢？每個插頭給你 **250V / 50A**，只能撐 **4 個節點**。升級到 H100 呢？**只剩 2 個**。
> **一個 50 安培的粗插頭，只能餵一個 H100 節點。**」

> 【口頭】所以在 AWS / GCP / Azure 上租 GPU 這麼貴，**有一部分成本其實是在付電費**。
> 教授的呼籲：「身為 MIT 的學生，我們有責任用 **green AI** 保護這個世界。」

#### 【口頭】另外幾個具體場景

| 場景 | 教授講的例子 |
|---|---|
| **Storage** | 想創業做有 AI 的 App？你**不會想把 175B 參數的模型丟上 App Store**。FP16 = 350 GB，使用者一下載就把整個月的流量燒光 |
| **OTA 更新** | 開 Tesla 的話，模型是**空中更新（over-the-air）**的，不用回原廠——但那也是在吃 LTE 頻寬 |
| **Communication** | InfiniBand 網卡 400 Gbps × 8 張 GPU，聽起來很多，但模型一大還是不夠；而且**每一條 InfiniBand 線就要約 1,000 美元** |

這些維度手工難以同時最佳化，於是有了「**用 AI 設計更好的 AI**」。
...
> 【口頭】教授在課堂上的說法比較保守：「兩者**大致相同，稍微多一點計算，也稍微多一點參數**。」
> 也就是說 —— **inverted bottleneck 在算術上並沒有比較省**，這點值得記住。
...
> 【口頭】**peak activation 的精確定義**：它**不是** total activation，而是「**該層的輸入 activation + 輸出 activation**」
> —— 也就是**你必須同時擺在 AI 晶片上的那一份**。
...
#### 【口頭】⭐ Depthwise 的興衰（這段投影片沒有，但很重要）

> 「**2016、2017 年的時候，depthwise 這類層被超級廣泛使用。**
> 但隨著時間過去，特別是 **GPU、DSP 這種高平行度硬體**越來越進步，**depthwise convolution 用得越來越少**。
> 反而是**普通的 dense 3×3 convolution 更有效率** —— 它能更好地吃到硬體的平行度，而且**計算模式更規則**。」
>
> 「所以**並不是永遠 depthwise 或 MobileNet 就比較高效。實際上有時候它們超級不高效。**」

**Take-away**：building block 的好壞**依硬體平台而定**，沒有普世最佳解。
（↔ 這正是 §8 hardware-aware NAS 要處理的問題，也是為什麼 §4.2 的骨幹後段換回了 dense 的 Attention/FFN。）
...
> 【口頭】投影片的 attention map 視覺化用的句子是 **"I thought the video game was a lot more fun than the film"**（每個字當一個 token）。
> 可以清楚看到 **video 高度 attend 到 game，game 也高度 attend 到 video** —— 「這樣的 attention map 是合理的」。
...
**缺點**【口頭】（教授特別強調）：機器設計出的 cell 連線極複雜，人難以理解，而且**依賴關係盤根錯節導致 buffer 無法提早釋放**，記憶體 footprint 比理論值大，硬體 mapping 效率差。

> #### 【Q&A】為何每個 block 固定取兩個 operand？
>
> **學生問**：為什麼一次挑兩層再合併？是為了避開某種 local minima 嗎？
>
> **教授答**：**「不是，這純粹是經驗性的。」** 你要三個、四個 operand 也可以，沒有非得是兩個的理由。
> 直覺上 add 是加兩個數、multiply 是乘兩個數、concat 是接兩塊 activation ——**二元本來就很自然**。
>
> **學生追問**：那這樣只是「兩層 + skip connection」，這根本不是完整的設計空間吧？
>
> **教授答**：**「確實不是完整的設計空間。」** skip connection 已經包含在裡面了（兩個 op 都選 identity 就是）。
> 但他直接承認：**「這個工作非常先驅，可是也有很多限制和假設。」**
...
**3×3 vs 5×5 的取捨**【口頭】（課堂討論）：兩層 3×3 與一層 5×5 感受野相同，但——
...
> #### 【Q&A】所以就是「隨機抽架構、再算 FLOPs」？
>
> **學生問**：做法就是隨機取樣一堆架構，然後算出 FLOPs 就好？
>
> **教授答**：**「完全正確。而且非常便宜 —— 你根本不用訓練它。」**
> （用 §1 的公式：conv 乘六項、FC 乘兩項，馬上就算得出來。若改用準確率當指標，就得真的把模型訓練起來，成本天差地別。）
>
> **另一則追問**：不同設計空間差在哪？
> **答**：它們**共用同一個記憶體約束**（例如 128 或 256 KB SRAM），差別在**解析度與寬度不同**，
> 於是在同樣的記憶體下能塞進的 FLOPs 就不一樣。

#### 【口頭】TinyNAS 的完整對照組（投影片有圖，但這段講解只在課堂上）

| 對照組 | 結果 |
|---|---|
| **TinyNAS 挑出的設計空間** | 最好 |
| **更大的 huge 設計空間** | **更差** —— 空間太大，在固定搜尋時間內找不到好東西 |
| **隨機挑的設計空間** | 明顯差 |
| **ResNet-18 @ 224** | **根本塞不進去（OOM）** |

> 【口頭】重點在於：搜出來的模型只用**幾百 KB 的 SRAM**，準確率卻能**逼近 ResNet-18 @ 224 解析度**。
...
### 5.6 【Q&A】能不能用 Graph Neural Network 來搜架構？

**學生問**（Lecture 7 最後）：既然架構可以表示成圖，能不能直接在上面跑 GNN？

**教授答**：**可以。** 神經網路本身就是一張計算圖，而 **GNN 很擅長萃取「距離較遠的節點之間的關係」**，所以把架構當圖、在上面做 NAS 是合理的。
要注意的是這是**有向圖**，而且**可能有環也可能無環（cyclic / acyclic 都可以）**——有些地方你不想重複，有些地方你就是想重複。

（↔ 這正好接到 §6.3 SMASH 用 GNN 做架構編碼，以及 §8.4 用 GNN 萃取特徵再預測 latency。）
...
> #### 【Q&A】那「只訓練 20 個 epoch 就評分」為什麼不行？
>
> **教授反問全班**：用少數幾個 epoch 的準確率當 proxy，問題在哪？
>
> **答**：**早期的排名不等於最終的排名。**
> - 有的模型**一開始收斂超快，但後面就沒潛力了**
> - 有的模型**一開始很慢，但後面 loss 突然一路狂掉**
>
> 所以「訓練幾個 epoch 就排序」會系統性地選錯模型。這是 §8.3 ProxylessNAS 要拋棄的四種 proxy 之一。
...
**學生問**：微小擾動不是「不應該」改變輸出嗎？這個分數不就在獎勵不穩定的模型？

**回應**：第二項就是為了壓住「對任何輸入都亂跳」的不穩定模型——BN 的作用正是穩定跨樣本的特徵分布。

**重要的邏輯限制**【口頭】（教授親自強調）：這類指標只能**排除**明顯的壞架構（輸入變了輸出不變 → 幾乎確定不好），**不能反推**「敏感 ⇒ 好」。
...
> #### 【Q&A】所以每一個候選架構都各有一份權重？
>
> **學生問**：這樣是不是每種可能的架構都要存一組權重？
>
> **教授答**：**每個 building block、每一條 path 各有自己的權重**（同一層可以重複很多次，不同層也可以有各自的選擇）。
> 而 **architecture parameter 是一個機率** —— 「在這四個選擇裡面，我該選這個 block 的機率是多少」。
>
> 更新 weight 時，**路徑是依這個機率取樣出來的**：機率高的路徑常被選到，
> 但**機率低的路徑仍然有機會被抽中**，只是次數少。
...
> #### 【Q&A】sample 子網路是均勻的，還是有加權？
>
> **學生問**：這裡的 sample 是 uniform 還是 weighted？
>
> **教授答**：**均勻隨機取樣（uniformly random sample）。**
> 而且他直接預告了 Lab 3 的順序：**先用 random sample 做第一階的 sanity check**（確認多抽幾次之後結果有在變好），
> **再實作演化演算法（crossover + mutation）來提升樣本效率**。（↔ §13.7、§13.8）

> 【口頭】**為什麼 Lab 3 跑得動免費版 Colab**：因為子網路**共享同一份超網路權重、完全不用訓練**。
> 教授說 Lab 1–3 他們刻意設計成免費版 Colab 就能跑完；**只有 Lab 4（壓縮大型語言模型）非得用付費版（$10/月）不可**——「我們真的盡力了」。

> 【口頭】OFA 要支援的場景不只是「新舊手機」，還包括**同一台手機的滿電模式 vs 省電模式**、**早上 vs 晚上**。
...
> #### 【Q&A】「跳過層」到底是怎麼跳的？
>
> **學生問**：可以解釋一下 elastic depth 是怎麼運作的嗎？
>
> **教授答**：就是**直接拿 O1 / O2 / O3 那一層的輸出，忽略後面的層**。
> 訓練時**每次迭代隨機取樣不同的深度**——例如各 **1/3 的機率**選 2 層、3 層或 4 層。
>
> **學生追問**：那維度不就對不上？
> **答**：**維度完全一樣。** 因為這是**同一個 stage、同一個解析度**內的事，差別只在「經過幾層」。
>
> ⭐ 教授強調的關鍵差異：**「每一次迭代，我們訓練的是網路的不同部分。」**
> 這跟訓練一個固定網路有本質上的不同。

> 【口頭】教授的類比：這種稀疏啟動的精神和 **Mixture-of-Experts** 相通——
> 他還提到當時的傳聞：**「最近關於 GPT-4 的傳聞裡，他們用的關鍵技術之一就是 mixture-of-experts」**，
> 一樣是**訓練時和測試時都只稀疏啟動一部分**。
>
> **學生問**：那算是某種「跨時間的 ensemble」嗎？
> **教授答**：**「對，可以這樣說」**——但它**不完全等同於 ensemble**。
...
> #### 【口頭】Roofline 圖怎麼讀
>
> - **x 軸 = Ops/Byte（arithmetic intensity）**：抓一個 byte 的資料進來，能在上面做多少次運算
> - **y 軸 = GOPS/s**：每秒實際做了多少 G 次運算
> - 左邊斜線段 = **memory bound**（頻寬吃滿、算力還閒著）；右邊平台 = **compute bound**（撞到硬體算力上限）
> - **越往右上越好**——因為「**搬資料很貴、算數很便宜**」，抓一次資料就要盡量多算，把搬運成本攤掉
>
> **【Q&A】** 有學生把 Ops/Byte 誤解成「同一批資料上取樣很多子網路」。
> **教授澄清**：**「這是低層次得多的概念。」** 指的是對一個給定的網路，抓權重、抓 activation，然後在上面做多少計算——跟取樣子網路無關。

> 【口頭】教授的感慨：「這個專案大概是**四年前**做的，用的顯卡還是 **1080 Ti**。
> 同樣是 **72.6% top-1**……我自己都有點驚訝，**現在大家已經在談 85% 了**，四年進步真大。」
...
拖動滑桿（改髮色 / 變年輕 / 拿掉眼鏡）
      ↓   跑【小子網路】→ 即時看到反應
按下 Finalize
      ↓   跑【完整模型】→ 最高品質，慢一點但只做這一次
```

> 【口頭】一個有趣的觀察：demo 裡「**變老**」這個方向，模型會**自動幫你加上眼鏡**——
> 因為在訓練資料裡這兩件事是相關的。把滑桿拉到最左邊，眼鏡就消失了。

> 【口頭】教授補充：**全部開源**，2021 年做的，當時就能在 MacBook 上本機跑；「現在的 MacBook 又比兩年前強多了」。

> **【Q&A】** 學生問：這些模型是離線先訓練好、再下載到 iPad 的嗎？
> **答**：**是的，全部都是推論時間的事。** 而且這正是 OFA 的價值——
> **裝置上只要存一個模型**，執行期再決定跑大的還是小的，不用下載大中小好幾份。
...
**做法**：一樣是訓練一個 once-for-all 模型再抽子網路。教授特別提到**單一路徑（single path）就已經表現得很好**，換來約 **5× 的延遲降低**，足以做**即時、多人**的姿態估計。

> #### 【口頭】一個學生的期末專案
>
> 「上學期有位同學用這個 demo 做了一個**划船訓練 app**——她在划船隊，用它來**分辨划船時的不同姿勢**，蠻有意思的。」
>
> 教授順便招手：**這可以是很好的期末專案方向，實驗室有幾支 Android 手機可以借。**
...
> 【口頭】**反直覺的一點**：**qubit 不是越多越好。**
> 更多 qubit 確實給你更大的容量，**但也帶來更多雜訊**。
> 所以「**量子電路的拓撲（topology）才是關鍵**」，不是塞越多電路就越好。

> 【口頭】那 **87% → 47%** 的落差怎麼來的：**87% 是無雜訊模擬（noise-free simulation）**，
> **47% 是在 IBM 真實量子機器上量到的結果**。中間的差距**全部來自雜訊**。
...
> 【口頭】super circuit 裡**每一個小方塊就是一個 gate**，取樣子電路就是決定「這個 gate 在不在」。
> 剪枝的理由很直接：**一個不必要的 gate 不只沒用，還會額外引入雜訊**，不如剪掉。
...
> ⚠️ 【投影片新增】**這張投影片是後續學期更新的內容**（Flextron 是 2024 年的論文），**2023 秋季的課堂逐字稿裡沒有這一段**。但它正好把整條主線收尾：
...
| 【口頭】一個 50A/250V 插頭能餵幾個節點 | 舊伺服器 10 台；A100 **4 個**；H100 **2 個** |
| 【口頭】InfiniBand 線材成本 | 每條約 **$1,000** |
| 【口頭】LitePose 延遲降低 | **約 5×**（single path 就夠好） |
| 【口頭】QuantumNAS 的 87% 從哪來 | **87% = 無雜訊模擬**，**47% = IBM 真機實測** |
| 【口頭】Anycost GAN 壓縮前「變年輕」耗時 | **約 3 秒** |
...
*筆記依據 MIT 6.5940 Fall 2023 Lecture 7（[錄影](https://youtu.be/W84MKJSg90A)）與 Lecture 8（[錄影](https://youtu.be/MOMc8KkGwCc)）的課堂逐字稿與官方投影片 `Lec07-Neural-Architecture-Search-I.pdf` / `Lec08-Neural-Architecture-Search-II.pdf` 整理，並對照 `Lab3_zh.md` 與 `hw3.py`。標記【口頭】處為投影片沒有、僅課堂講述的內容；【Q&A】為學生提問；【投影片新增】處為後續學期更新的投影片，2023 課堂逐字稿未涵蓋。*
